...
static int bcmgenet_probe(struct platform_device *pdev)
{
	...
	priv->clk = devm_clk_get_optional(&priv->pdev->dev, "sw_genet");
	if (IS_ERR(priv->clk)) {
	...
	priv->clk_wol = devm_clk_get_optional(&priv->pdev->dev, "sw_genetwol");
	if (IS_ERR(priv->clk_wol)) {
		err = PTR_ERR(priv->clk_wol);
		goto err_clk_disable;
	}

	priv->clk_eee = devm_clk_get_optional(&priv->pdev->dev, "sw_geneteee");
